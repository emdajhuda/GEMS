...
This diagram shows how the sub-packages under [`src/ocote/`](../src/ocote/) fit
...
```mermaid
flowchart TD
    A["LSST Butler\n(remote repository, e.g. LSSTComCam/DP1)"] -->|"ExpButler / LocalButler\nocote.utils.butler"| B["Local Butler repository"]

    subgraph Select["Visit & exposure selection — ocote.shortcuts"]
        C["VisitSH / combine_visits_selected\nvisit_sh"]
    ...
    subgraph Diagnostics["Diagnostics & plotting — ocote.utils.plot"]
        I["StatisticsPlots"]
...
1. **Data access** (`ocote.utils.butler`) — `ExpButler` wraps a remote LSST `Butler`;
...
2. **Selection helpers** (`ocote.shortcuts`) — `visit_sh` selects visits by sky
   position/tract-patch and combines them; `exposure_sh` loads, saves, normalizes, and
   cuts out exposures.

3. **Core processing**:
   - `ocote.coaddmaker` builds custom, filter-aware coadds from a selected set of
   ...
   - `ocote.coherentinjection` injects synthetic strong-lensing sources into exposures
     (via `lsst.source.injection`) to test recovery/detectability.
   - `ocote.utils.warp` re-projects ("warps") exposures onto a common sky grid ahead of
     coaddition.

4. **Diagnostics** (`ocote.utils.plot`) — turns the outputs of the layers above into
...
Cutting across all of the above, `ocote.utils.sky` (WCS and sky-coordinate math),
`ocote.utils.fits` (FITS <-> `Exposure` conversion), and `ocote.utils.tools` (logging,
